...
type WithdrawalOrderRepo interface {
	AddWithdrawalOrder(ctx context.Context, user *entity.User, form *request.UserWithdrawOrderForm) error
	GetWithdrawals(ctx context.Context, user *entity.User) ([]entity.WithdrawalOrder, error)
}
...
func GetWithdrawalOrderRepository() WithdrawalOrderRepo {
	or := new(WithdrawalOrderRepository)
	or.Conn = connection.Conn
	return or
}

func (or *WithdrawalOrderRepository) AddWithdrawalOrder(ctx context.Context, user *entity.User, form *request.UserWithdrawOrderForm) error {
	...
	if form.Sum > balance {
		...
		return &entity.ErrNotEnoughMemory{}
	}
...
func (or *WithdrawalOrderRepository) GetWithdrawals(ctx context.Context, user *entity.User) ([]entity.WithdrawalOrder, error) {
	...
	orderList := make([]entity.WithdrawalOrder, 0)

	for rows.Next() {
		var order entity.WithdrawalOrder
